...
from gui.ui.start import Ui_Form
from lxml import etree

class StartWin(QtWidgets.QWidget):
    single_push_result = pyqtSignal(str)
    single_push_result_temp = pyqtSignal(str)
    ...
    def _get_data(self,key):
        ...
        url = "https://jaywcjlove.gitee.io/linux-command/c/{}.html".format(key)
        response = requests.get(url, headers=headers)
        response.encoding="utf-8"
        data = self.parse_html(response.text)
        self.single_push_result.emit(data)

    def parse_html(self,html):
        tree = etree.HTML(html)
        if "404 Not Found" not in html:
            markdown = tree.xpath('//div[@class="markdown-body"]')[0]
            markdownbody = etree.tostring(markdown, encoding="unicode").replace("纠正错误", "").replace("添加实例", "")
            return "<!DOCTYPE html><html><head></head><body>{}</body></html>".format(markdownbody)
        else:
            return """<html>
...
<hr><center>openresty</center>
    ...
    def _init_slot(self):
        ...
        """
        self.ui.pushButton_search.clicked.connect(self.slot_pushButton_search)
        self.single_push_result.connect(self.show_result)

        self.ui.lineEdit_input.returnPressed.connect(self.ui.pushButton_search.click)

        self.ui.lineEdit_input.textChanged.connect(self.slot_textChanged)

        self.single_push_result_temp.connect(self.slot_single_push_result_temp)

    def slot_single_push_result_temp(self,text):
        self.ui.textEdit_out.append(text)

    def slot_textChanged(self,text):
        print("text =", text)
        if text:
            self.ui.textEdit_out.clear()
            text_out = []
            for args in self.select_temp:
                if text in args["n"]:
                    text_out.append("{}    {}".format(args["n"],args["d"]))
            self.single_push_result_temp.emit("\n".join(text_out))
        else:
            self.ui.textEdit_out.clear()

    def show_result(self,msg):
        self.ui.textEdit_out.setHtml("{}".format(msg))

    def slot_pushButton_search(self):
        """
        """
        args = self.ui.lineEdit_input.text().strip()
        if args:
            self.ui.textEdit_out.setText("搜索中…………")
            self.pool.submit(self._get_data,
                             args.strip(),
                             )
        else:
            self.single_push_result.emit("请输入搜索参数")



    def closeEvent(self, event):
        reply = QMessageBox.question(self, '警告','<font color=red>窗口关闭后，将终止本次运行</font>',
